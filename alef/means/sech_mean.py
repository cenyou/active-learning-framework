...
def sech(x):
    return 2*tf.exp(x) / (1.0 + tf.exp(2*x))

class SechMean(MeanFunction):
    def __init__(self, input_size, **kwargs):
        ...
        weights = 10*np.ones([input_size, 1], dtype=default_float())
    ...
    def __call__(self, X: TensorType) -> tf.Tensor:
        ...
        q = sech( tf.einsum('...ND,Dm->...Nm', shifted_input, self.weights) )
