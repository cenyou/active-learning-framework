...
class PeriodicMean(MeanFunction):
    def __init__(self, input_size, **kwargs):
        ...
        weights = 8*np.ones([input_size, 1], dtype=default_float())
        self.weights = Parameter(weights)

        self.scale = Parameter(3.0*np.ones(1, dtype=default_float()))
        self.bias = Parameter(np.zeros(1, dtype=default_float()))

    def __call__(self, X: TensorType) -> tf.Tensor:
        # input is [*B, N, D], center / weights is [*B, D], up to broadcast permit
        shifted_input = 1/self.center.shape[-1] * (X - self.center)**2
        
        q = tf.cos( tf.einsum('...ND,Dm->...Nm', shifted_input, self.weights) )
