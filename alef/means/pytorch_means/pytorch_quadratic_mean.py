...
class QuadraticPytorchMean(WrapMean):
    def __init__(
        self,
        input_size,
        ...
        **kwargs
        ...
        self.register_parameter(
            name='raw_weights',
            parameter=torch.nn.Parameter( 3*torch.ones([*batch_shape, input_size], **pass_kwargs) )
        )
        ...
        self.register_parameter(
            name='scale',
            parameter=torch.nn.Parameter( -10*torch.ones([*batch_shape], **pass_kwargs) )
        )
    ...
    def _set_weights(self, value):
        ...
        self.raw_weights.data = self.raw_weights_constraint.inverse_transform(value)
    ...
    def forward(self, input, mask=None):
        ...
        if mask is None:
            q = (self.weights.unsqueeze(-2).expand(shape) * shifted_input**2).mean(-1)
        else:
            m = mask.unsqueeze(-2).expand(shape)
            shifted_input = shifted_input.masked_fill(m==0, 0)
            q = (self.weights.unsqueeze(-2).expand(shape) * shifted_input**2).sum(-1) / m.sum(-1)
...
if __name__ == '__main__':
    ...
    D = 2
    B = 3
    m = QuadraticPytorchMean(D, [])
    x = torch.rand([4, B, 10, D])
    print(m(x).shape)
    x = torch.rand([1000, D])
    
    print(m.state_dict()['center'])
