...
class PeriodicPytorchMean(WrapMean):
    def __init__(
        self,
        input_size,
        ...
        **kwargs
        ...
        """
        m(x) = c * ( b + cos( 1/d sum_d w_d(x_d - center_d)**2 ) )
        """
        super().__init__()
        ...
        self.register_parameter(
            name='weights',
            parameter=torch.nn.Parameter( 8*torch.ones([*batch_shape, input_size], **pass_kwargs) )
        )
        ...
        self.register_parameter(
            name='bias',
            parameter=torch.nn.Parameter( torch.zeros([*batch_shape], **pass_kwargs) )
        )
    ...
    def forward(self, input, mask=None):
        ...
        if mask is None:
            q = torch.cos( (self.weights.unsqueeze(-2).expand(shape) * shifted_input**2).mean(-1) )
        else:
