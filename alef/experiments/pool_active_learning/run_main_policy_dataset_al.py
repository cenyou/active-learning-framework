...
from alef.experiments.pool_active_learning.main_policy_dataset_al import experiment
...
for dataset, configs in test_dataset_metadata.items():
    ...
    for exp_id in range(5):
        for p in EXPERIMENT_PATH.glob(f'amortized_AL/ContinuousGP{d}DPolicy_loss_*/*/'):
            ...
            exp_args = exp_arguments(None, EXPERIMENT_PATH / 'data', exp_id, p, dataset, wish_to_plot, eval_gap, n_init, n_steps, n_test)
            try:
                experiment(exp_args)
