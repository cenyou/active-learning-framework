...
class MultitaskPoolFromPools(BasePool, BasePoolWithSafety):
    """
    ...
    def __init__(
        ...
        target_pool = pool_list[-1]
        assert all([isinstance(pool, BasePool) for pool in pool_list]) or all([isinstance(pool, BasePoolWithSafety) for pool in pool_list])
    ...
    def set_task_mode(self, pool_index: int):
        assert pool_index < self.output_dimension, f'{pool_index} must be smaller than {self.output_dimension}'
        assert pool_index >= 0, f'{pool_index} must be non-negative'
