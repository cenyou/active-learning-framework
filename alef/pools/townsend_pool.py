...
from alef.oracles import OracleNormalizer, TownsendMain, TownsendConstraint

class TownsendPool(PoolWithSafetyFromOracle):
    def __init__(self, observation_noise: float, seed: int=123, set_seed: bool=False):
        oracle = OracleNormalizer(TownsendMain(observation_noise))
        oracle.set_normalization_by_sampling()

        safety_oracle = OracleNormalizer(TownsendConstraint(observation_noise))
