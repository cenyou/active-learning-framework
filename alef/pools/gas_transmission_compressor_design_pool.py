...
from alef.oracles import OracleNormalizer, GasTransmissionCompressorDesignMain, GasTransmissionCompressorDesignConstraint

class GasTransmissionCompressorDesignPool(PoolWithSafetyFromOracle):
    def __init__(self, observation_noise: float, seed: int=123, set_seed: bool=False):
        oracle = OracleNormalizer(GasTransmissionCompressorDesignMain(observation_noise))
        oracle.set_normalization_by_sampling()

        safety_oracle = OracleNormalizer(GasTransmissionCompressorDesignConstraint(observation_noise))
