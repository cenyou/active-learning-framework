...
class PoolWithSafetyFromConstrainedOracle(BasePoolWithSafety):
    def __init__(
        self,
        oracle: StandardConstrainedOracle,
        seed: int=123,
        set_seed: bool=False
    ):
        assert isinstance(oracle, StandardConstrainedOracle), NotImplementedError
        self.oracle = oracle
        _, _, Z = oracle.get_random_data(1, noisy=False)
        super().__init__(safety_dimension=Z.shape[-1])
    ...
    def get_grid_data(self, n_per_dim:int, noisy:bool=True):
        ...
        return self.oracle.get_grid_data(n_per_dim, noisy)

    def get_random_data(self, n, noisy=True):
        return self.oracle.get_random_data(n, noisy)
