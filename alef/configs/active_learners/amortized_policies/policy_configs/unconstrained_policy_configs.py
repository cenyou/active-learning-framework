...
"""
class ContinuousGP2DPolicyConfig(ContinuousGPPolicyConfig):
    input_dim: int = 2
    observation_dim: int = 1
    hidden_dim_encoder: Sequence[int] = [512]
    encoding_dim: int = 32
    hidden_dim_emitter: Sequence[int] = [512]
    num_self_attention_layer: int=2
    name: str = 'basic_continuous_gp2d_al_policy_config'
"""
class ContinuousGPFlexDimPolicyConfig(BaseAmortizedPolicyConfig):
    encoding_dim: int = 32
    num_self_attention_layer: int=2
    ...
    domain_warpper: DomainWarpperType=DomainWarpperType.TANH
    device: str = 'cpu'
    name: str = 'continuous_gp_al_flexible_dimension_policy_config'
