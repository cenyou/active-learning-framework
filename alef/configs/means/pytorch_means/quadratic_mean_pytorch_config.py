...
class BasicQuadraticMeanPytorchConfig(BaseMeanPytorchConfig):
    input_dimension: int
    center: Union[float, Sequence[float]] = (INPUT_DOMAIN[1] - INPUT_DOMAIN[0]) / 2
    weights: Union[float, Sequence[float]] = 10.0
    scale: float = -2.0
    bias: float = -0.5
    center_prior: Optional[Tuple[GPytorchPriorEnum, Sequence]] = None # (prior_type, [*args, **kwargs])
    weights_prior: Optional[Tuple[GPytorchPriorEnum, Sequence]] = None
    scale_prior: Optional[Tuple[GPytorchPriorEnum, Sequence]] = None
    bias_prior: Optional[Tuple[GPytorchPriorEnum, Sequence]] = None
    name: str = "QuadraticMean"