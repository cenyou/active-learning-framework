...
from .base_safe_active_learners import BaseOracleSafeActiveLearner

class OraclePolicySafeActiveLearner(BaseOracleSafeActiveLearner):
    ...
    def __init__(
        ...
        train_with_safe_data_only: bool=False,
        *,
    ...
    def update_gp_model(self):
        ...
        safe_mask = np.all(safety_observations >= 0, axis=1) if self.train_with_safe_data_only else np.ones([self.x_data.shape[0]], dtype=bool)
    ...
    def update_safe_gp_models(self):
        safe_mask = np.all(self.z_data >= 0, axis=1) if self.train_with_safe_data_only else np.ones([self.x_data.shape[0]], dtype=bool)
    ...
    def update(self, remaining_budget: int=None):
        ...
        return self.run_policy(self.x_data, self.y_data, z, remaining_budget)
    ...
    def learn(self, n_steps: Union[int, List[int]]):
        ...
        for i in range(0, self.n_steps):
            ...
            if self.constraint_on_y:
                ...
                new_y, new_z = self.oracle.query(query)
    ...
    def plot(self, query: np.array, new_y: float, new_z: float, n_step: int):
        ...
        z_plot = y_plot if self.constraint_on_y else self.gt_safety_values
        if dimension == 1:
