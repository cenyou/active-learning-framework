...
class PoolPolicyActiveLearner(BasePoolActiveLearner):
    ...
    def run_policy(self, x_data: np.ndarray, y_data: np.ndarray, remaining_budget: int=None):
        ...
        if self.policy.forward_with_budget:
            ...
            query = self.policy(T, X, Y).cpu().detach().numpy() # [1, D]
        else:
            query = self.policy(X, Y).cpu().detach().numpy() # [1, D]
    ...
    def update(self, remaining_budget: int=None):
        ...
        query = self.run_policy(self.x_data, self.y_data, remaining_budget).reshape([1, D])
    ...
    def learn(self, n_steps: Union[int, List[int]]):
        ...
        for i in range(0, self.n_steps):
            ...
            #if self.do_plotting and self.pool.get_dimension() <=2:
            #    need_gp = True
            #elif self.validation_at is None or len(self.validation_at)==0 or i in self.validation_at:
            #    need_gp = True
