...
from alef.models.amortized_infer_structured_kernels.data_generators.dataset_of_datasets import RandomDatasetOfDatasets
from alef.models.amortized_infer_structured_kernels.data_generators.generator_factory import GeneratorFactory
from alef.models.amortized_infer_structured_kernels.data_generators.simulator import Simulator
from alef.models.amortized_infer_structured_kernels.gp.utils import cal_marg_likelihood, cal_marg_likelihood_batch_noise
...
from alef.kernels.linear_kernel import LinearKernel as LinearKernelGPflow
from alef.kernels.periodic_kernel import PeriodicKernel as PeriodicKernelGPflow
from alef.configs.kernels.matern52_configs import BasicMatern52Config
from alef.models.amortized_infer_structured_kernels.gp.base_kernels import SEKernel
from alef.configs.kernels.rbf_configs import BasicRBFConfig
import pytest
import random
from alef.models.amortized_infer_structured_kernels.nn.noise_predictor_head import NoiseVariancePredictorHead
...
import copy
from alef.models.amortized_infer_structured_kernels.utils.utils import (
    default_worker_init_function,
    get_test_inputs,
)
import gpytorch
from alef.models.amortized_infer_structured_kernels.gp.base_kernels import N_BASE_KERNEL_TYPES
...
def test_dataset_encoding_masking():
    pass
...
def test_predict_amortized_model(d, n_train, n_test):
    ...
    model = DimWiseAdditiveKernelsAmortizedInferenceModel(
        dataset_encoder, kernel_encoder, True, [], 0.1, 1e-2, BaseKernelEvalMode.DEBUG
    )
    kernel_list = [[BaseKernelTypes.SE] for i in range(0, d)]
    mu_test, _, sigma_test = model.predict(X_test, X_data, y_data, kernel_list)
    kernel_config = BasicRBFConfig(input_dimension=d)
    ...
    kernel_config_torch = BasicRBFPytorchConfig(input_dimension=d)
    ...
    model_gpytroch = ModelFactory.build(model_config_torch)
    ...
    assert np.allclose(mu_test, mu_gpflow, atol=1e-4)
    assert np.allclose(sigma_test, sigma_gpflow, atol=1e-4)
...
def test_predict_amortized_model_warm_start(d, n_train, n_test, base_kernel_type):
    ...
    assert np.allclose(sigma_test, sigma_test2, rtol=1e-4, atol=1e-5)
    model.set_warm_start_params(20, 0.01)
    mu_test3, _, sigma_test3 = model.predict(X_test, X_data, y_data, kernel_list)
    assert not np.allclose(mu_test3, mu_test2, rtol=1e-4, atol=1e-5)
    assert not np.allclose(sigma_test3, sigma_test2, rtol=1e-4, atol=1e-5)
...
def test_predict_amortized_model_with_noise():
    pass
...
def test_input_equivariance_kernel_encoding():
    ...
    new_kernel_list_single_dimensions_permuted = [
        BaseKernelTypes.LIN,
    ...
    for kernel_encoder in kernel_encoder_list:
        ...
        kernel_encoder.eval()
        kernel_encoding_1 = (
            kernel_encoder.forward(kernel_embeddings1, dataset_encoding, kernel_mask_1, dim_mask).detach().numpy()
        )
        kernel_encoding_2 = (
            kernel_encoder.forward(kernel_embeddings2, dataset_encoding, kernel_mask_2, dim_mask).detach().numpy()
        )
        for index in range(0, len(permutation_indexes)):
            assert np.allclose(
                kernel_encoding_1[0, 0, index, :],
                kernel_encoding_2[0, 0, permutation_indexes[index], :],
                rtol=1e-4,
                atol=1e-5,
            )
...
def test_kernel_list_to_expression_transformation():
    kernel_list = [
        [BaseKernelTypes.SE, BaseKernelTypes.LIN, BaseKernelTypes.LIN],
        [BaseKernelTypes.SE_MULT_PER, BaseKernelTypes.PER],
    ]
...
def test_kernel_list_to_expression_transformation2():
    ...
    se_on_0 = ElementaryKernelGrammarExpression(
        RBFKernel(**BasicRBFConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=0).dict())
    )
    lin_on_0 = ElementaryKernelGrammarExpression(
        LinearKernelGPflow(
            **BasicLinearConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=0).dict()
        )
    )
    per_on_0 = ElementaryKernelGrammarExpression(
        PeriodicKernelGPflow(
            **BasicPeriodicConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=0).dict()
        )
    )
    se_on_1 = ElementaryKernelGrammarExpression(
        RBFKernel(**BasicRBFConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=1).dict())
    )
    lin_on_1 = ElementaryKernelGrammarExpression(
        LinearKernelGPflow(
            **BasicLinearConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=1).dict()
        )
    )
    per_on_1 = ElementaryKernelGrammarExpression(
        PeriodicKernelGPflow(
            **BasicPeriodicConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=1).dict()
        )
    )
    lin_on_2 = ElementaryKernelGrammarExpression(
        LinearKernelGPflow(
            **BasicLinearConfig(input_dimension=3, active_on_single_dimension=True, active_dimension=2).dict()
        )
    )
    ...
    expression_dim_0 = KernelGrammarExpression(
        KernelGrammarExpression(se_on_0, lin_on_0, KernelGrammarOperator.ADD),
        lin_mult_per_on_0,
        KernelGrammarOperator.ADD,
    )
    ...
    expression2 = transform_kernel_list_to_expression(kernel_list, use_gpflow=False)
    X = np.random.rand(20, 3)
    X2 = np.random.rand(10, 3)
    X_torch = torch.from_numpy(X)
    X_torch2 = torch.from_numpy(X2)
    gpflow_kernel = expression.get_kernel()
    gpytorch_kernel = expression2.get_kernel()
    K1 = gpflow_kernel(X, X2).numpy()
    K2 = gpytorch_kernel(X_torch, X_torch2).numpy()
    assert np.allclose(K1, K2)


def test_invariances_end_to_end():
    pass
...
def test_input_invariance_dataset_encoding():
    ...
    for dataset_encoder in dataset_encoder_list:
        ...
        X_padded_perm, y_padded_perm, size_mask_perm, dim_mask_perm, _, _ = get_padded_dataset_and_masks(
            [X_perm], [y_perm]
        )
        ...
        assert np.allclose(output.detach().numpy(), output_perm.detach().numpy(), rtol=1e-4, atol=1e-5)
        X_dim0 = X[:, 0]
        X_dim2 = X[:, 2]
        X_dim_permute = X.copy()
        X_dim_permute[:, 0] = X_dim2
        X_dim_permute[:, 2] = X_dim0
        X_padded_dim_perm, y_padded_dim_perm, size_mask_dim_perm, dim_mask_dim_perm, _, _ = (
            get_padded_dataset_and_masks([X_dim_permute], [y])
        )
        ...
        assert np.allclose(output_dim_perm_perm, output.detach().numpy(), rtol=1e-4, atol=1e-5)
...
def test_simulated_dataset_consistency(sample_noise: bool):
    ...
    torch.from_numpy(X)
    torch.squeeze(torch.from_numpy(y))
...
def test_simulated_dataset_storing_and_loading():
    pass
...
def test_dim_wise_additive_kernel_wrapper2():
    ...
    for i in range(0, n_repeats):
        ...
        kernel_expression_gpflow = transform_kernel_list_to_expression(kernel_list, use_gpflow=True)
        kernel_embeddings, kernel_mask = get_kernel_embeddings_and_kernel_mask([kernel_list])
        ...
        K_gpflow = kernel_expression_gpflow.get_kernel().K(X, X2)
        K_gpytorch = kernel_expression.get_kernel()(X_torch, X_torch2).detach().numpy()
        assert np.allclose(K_torch.numpy(), K_gpflow.numpy())
...
def test_dim_wise_additive_kernel_wrapper():
    ...
    per_on_0 = ElementaryKernelGrammarExpression(
        PeriodicKernelGPflow(
            **BasicPeriodicConfig(
                input_dimension=3, active_on_single_dimension=True, active_dimension=0, base_period=1.0
            ).dict()
        )
    )
    se_on_1 = ElementaryKernelGrammarExpression(
    ...
    per_on_1 = ElementaryKernelGrammarExpression(
        PeriodicKernelGPflow(
            **BasicPeriodicConfig(
                input_dimension=3, active_on_single_dimension=True, active_dimension=1, base_period=1.0
            ).dict()
        )
    )
    lin_on_2 = ElementaryKernelGrammarExpression(
    ...
    additive_wrapper = DimWiseAdditiveKernelWrapper(N_BASE_KERNEL_TYPES, [], 0.1, True, BaseKernelEvalMode.DEBUG)
    K_torch = additive_wrapper.forward(X_torch, X_torch2, kernel_embeddings, kernel_list)
    K_gpflow = expression.get_kernel().K(X, X2)
    assert np.allclose(K_torch.numpy(), K_gpflow.numpy())


def test_marginal_likelihood():
    X_list, y_list, X_padded, y_padded, N, size_mask, _, kernel_embeddings, _, size_mask_kernel, kernel_list = (
        get_test_inputs(5, 10, 4, 1, only_SE=True)
    )
    ...
    mll = (
        cal_marg_likelihood(K, y_padded.unsqueeze(-1), 1e-2, size_mask_kernel, 1.0 - size_mask, N, torch.device("cpu"))[
            0
        ]
        * N
    )
    ...
    for i, x_data in enumerate(X_list):
        ...
        mll_i = model.model.log_marginal_likelihood().numpy()
        mlls_gpflow.append(mll_i)
    assert np.allclose(np.array(mlls_gpflow), mll, atol=1e-4, rtol=1e-4)
...
def test_kernel_expression_kernel_list_alignment():
    ...
    combined_expression_2 = KernelGrammarExpressionTransformer.multiply_expressions(
        kernel_expression_list, make_deep_copy=False
    )
    kernel_3 = get_gpytorch_kernel_from_expression_and_state_dict(
        combined_expression, kernel_state_dict, wrap_in_addition=True
    )
...
def test_marginal_likelihood_extented():
    ...
    model.get_num_predicted_params(kernel_list)
    mll = -1 * nmll * N
    mll_with_prior = -1 * nmll_with_prior * N
    mlls_gpflow = []
    mlls_with_prior_gpflow = []
    for i, x_data in enumerate(X_list):
        y_data = y_list[i]
        x_data.shape[0]
        d = x_data.shape[1]
        ...
        model.infer(x_data, np.expand_dims(y_data, axis=1))
        mll_i = model.model.log_marginal_likelihood().numpy()
        mll_with_prior_i = model.model.log_posterior_density().numpy()
        mlls_gpflow.append(mll_i)
        mlls_with_prior_gpflow.append(mll_with_prior_i)
    assert np.allclose(np.array(mlls_gpflow), mll, rtol=1e-3, atol=1e-2)
    assert np.allclose(np.array(mlls_with_prior_gpflow), mll_with_prior, rtol=1e-3, atol=1e-2)
...
def test_base_on_single_dims_kernels(dim, active_dim):
    X = np.random.rand(20, dim)
    X2 = np.random.rand(10, dim)
    X_torch = torch.from_numpy(X)[:, active_dim].unsqueeze(-1)
    X_torch2 = torch.from_numpy(X2)[:, active_dim].unsqueeze(-1)
    gpflow_kernel_config = BasicRBFConfig(input_dimension=dim)
    gpflow_kernel_config.active_on_single_dimension = True
    gpflow_kernel_config.active_dimension = active_dim
    gpflow_kernel_config.base_lengthscale = 0.7
    gpflow_kernel_config.base_variance = 0.25
    ...
    K_torch = se_kernel.forward(X_torch, X_torch, None)
    K_gpflow = KernelFactory.build(gpflow_kernel_config).K(X, X)
    assert np.allclose(K_torch.numpy(), K_gpflow.numpy())
    K_torch2 = se_kernel.forward(X_torch, X_torch2, None)
    K_gpflow2 = KernelFactory.build(gpflow_kernel_config).K(X, X2)
    assert np.allclose(K_torch2.numpy(), K_gpflow2.numpy())

    gpflow_kernel_config = BasicPeriodicConfig(input_dimension=dim)
    gpflow_kernel_config.active_on_single_dimension = True
    gpflow_kernel_config.active_dimension = active_dim
    gpflow_kernel_config.base_lengthscale = 0.7
    gpflow_kernel_config.base_variance = 0.25
    gpflow_kernel_config.base_period = 0.5
    ...
    K_torch = per_kernel.forward(X_torch, X_torch, None)
    K_gpflow = KernelFactory.build(gpflow_kernel_config).K(X, X)
    assert np.allclose(K_torch.numpy(), K_gpflow.numpy())
    K_torch2 = per_kernel.forward(X_torch, X_torch2, None)
    K_gpflow2 = KernelFactory.build(gpflow_kernel_config).K(X, X2)
    assert np.allclose(K_torch2.numpy(), K_gpflow2.numpy())

    gpflow_kernel_config = BasicLinearConfig(input_dimension=dim)
    gpflow_kernel_config.active_on_single_dimension = True
    gpflow_kernel_config.active_dimension = active_dim
    lin_kernel = LinearKernel(4, [], 0.1, BaseKernelEvalMode.DEBUG)
    K_torch = lin_kernel.forward(X_torch, X_torch, None)
    K_gpflow = KernelFactory.build(gpflow_kernel_config).K(X, X)
    assert np.allclose(K_torch.numpy(), K_gpflow.numpy())
    K_torch2 = lin_kernel.forward(X_torch, X_torch2, None)
    K_gpflow2 = KernelFactory.build(gpflow_kernel_config).K(X, X2)
    assert np.allclose(K_torch2.numpy(), K_gpflow2.numpy())

    gpflow_kernel_config = BasicMatern52Config(input_dimension=dim)
    gpflow_kernel_config.active_on_single_dimension = True
    gpflow_kernel_config.active_dimension = active_dim
    gpflow_kernel_config.base_lengthscale = 0.7
    gpflow_kernel_config.base_variance = 0.25
    ...
    K_torch = matern_kernel.forward(X_torch, X_torch, None)
    K_gpflow = KernelFactory.build(gpflow_kernel_config).K(X, X)
    assert np.allclose(K_torch.numpy(), K_gpflow.numpy())
    K_torch2 = matern_kernel.forward(X_torch, X_torch2, None)
    K_gpflow2 = KernelFactory.build(gpflow_kernel_config).K(X, X2)
    assert np.allclose(K_torch2.numpy(), K_gpflow2.numpy())


def test_base_kernels_on_mult_dims():
    pass
...
def test_noise_predictor_shapes_and_masking():
    ...
    (
        ...
        size_mask_kernel,
        kernel_list,
    ) = get_test_inputs(batch_size, 10, 4, 1, only_SE=True)
...
def test_batch_noise_marginal_likelhood():
    ...
    mll = (
        cal_marg_likelihood_batch_noise(
            ...
            torch.tensor(noise_variances).float(),
            size_mask_kernel,
            1.0 - size_mask,
            N,
            torch.device("cpu"),
    ...
    for i, x_data in enumerate(X_list):
        ...
        model_config = BasicGPModelConfig(kernel_config=kernel_config)
        model_config.optimize_hps = False
        model_config.observation_noise = np.sqrt(np.squeeze(noise_variances[i]))
        model = ModelFactory.build(model_config)
        # model.kernel = gpflow.kernels.RBF()
        model.infer(x_data, np.expand_dims(y_data, axis=1))
        mll_i = model.model.log_marginal_likelihood().numpy()
        mlls_gpflow.append(mll_i)
    assert np.allclose(np.array(mlls_gpflow), mll)
...
def test_shapes_of_to_input_tensor_of_dim_wise_additive():
    pass


if __name__ == "__main__":
    test_predict_amortized_model(2, 30, 20)
